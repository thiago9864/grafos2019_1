...
GrafoMatriz::GrafoMatriz(int n_nos) {

    this->inicializa_matrix(n_nos);

}

GrafoMatriz::GrafoMatriz(string entrada, string saida) {
...
GrafoMatriz::GrafoMatriz(string entrada, string saida, bool direcional) {
...
GrafoMatriz::GrafoMatriz(string entrada, string saida, bool direcional, bool ponderadoAresta) {
    this->direcional = direcional;
    this->ponderadoNo = false;
...
GrafoMatriz::GrafoMatriz(string entrada, string saida, bool direcional, bool ponderadoAresta, bool ponderadoNo) {
    this->direcional = direcional;
...
void GrafoMatriz::imprime(string arquivo) {
...
void GrafoMatriz::setAresta(int idOrigem, int idFim, int peso) {
    ...
    // Verificando se pode atualizar grau do GrafoMatriz
...
void GrafoMatriz::setAresta(int idOrigem, int idFim) {
    this->setAresta(idOrigem, idFim, 1);
}

/// Retorna peso da aresta em dada posição da matriz.
int GrafoMatriz::getArestaPos(int i, int j) {
    return this->matrix[i][j];
}

/// Retorna peso da aresta dado id do vertice de origem e fim.
int GrafoMatriz::getArestaId(int origem, int destino) {
    int posOrigem = this->findId(origem);
    int posDestino = this->findId(destino);

    return this->matrix[posOrigem][posDestino];

}

/// Retorna grau de entrada de um nó passado.
int GrafoMatriz::getGrauNoEntrada(int id){
...
int GrafoMatriz::getGrauNoSaida(int id){
 ...
 */
void GrafoMatriz::inicializa_matrix(int n_nos) {
    ...
    this->matrix = new int*[ordem];
    for (int i = 0; i < ordem; i++) {
        this->matrix[i] = new int[ordem];
...
void GrafoMatriz::leitura_arquivo(string arquivo) {
...
int GrafoMatriz::idPos(int pos) {
    return this->ids[pos];
}

/// Retorna ultima posição de um vetor
int GrafoMatriz::lastPos(int *v) {
    int i = 0;
...
void GrafoMatriz::atualizaGrau(int grau) {
    if (grau > this->grau)
        this->grau = grau;
}